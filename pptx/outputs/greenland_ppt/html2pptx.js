...
async function getBodyDimensions(page) {
  ...
  const errors = [];
  const widthOverflowPx = Math.max(0, bodyDimensions.scrollWidth - bodyDimensions.width - 5);
  const heightOverflowPx = Math.max(0, bodyDimensions.scrollHeight - bodyDimensions.height - 5);
...
function addElements(slideData, targetSlide, pres) {
  for (const el of slideData.elements) {
    if (el.type === 'image') {
      ...
      if (el.animation) imgOpts.animation = el.animation;
      ...
      if (el.animation) shapeOptions.animation = el.animation;
      ...
      if (el.animation) listOptions.animation = el.animation;
      ...
      if (el.animation) textOptions.animation = el.animation;
...
async function extractSlideData(page) {
  return await page.evaluate(() => {
    ...
    const getAnimation = (el) => {
      const type = el.getAttribute('data-animate-type');
      if (!type) return null;

      const anim = { type: type };
      const duration = el.getAttribute('data-animate-duration');
      const delay = el.getAttribute('data-animate-delay');

      if (duration) anim.duration = parseInt(duration);
      if (delay) anim.delay = parseInt(delay);
